...
$(function () {
  if (!isGitCodeBlog) {
    ...
    var getKeywordForEdu = function getKeywordForEdu(eduArr) {
      var keywords = {};
      // var keyword = 'Markdown 欢迎';
      // var arr = ['python', 'opencv'];
      var arrMap = new Map();
      eduArr.map(function (item) {
        return arrMap.set(item.words.toLowerCase().replace(/(^\s*)|(\s*$)/g, ""), item);
      });
      var arr = [].concat(_toConsumableArray(arrMap.keys()));
      var keyword = arr.join(",");
      mergeKeyword(keywords, keyword);
      var keyword_list = Object.keys(keywords);
      keyword_list.sort(function (x, y) {
      ...
      var result = new Map();
      keyword_list.map(function (item) {
        return result.set(item, arrMap.get(item));
      });
      return result;
    };
    ...
    var hlDocumentForEdu = function hlDocumentForEdu(eduArr, keyword_list, document) {
      var body = document.html();
      var htmlBody = '';
      var textChildrenArr = document.context.childNodes;
      for (var idx = 0; idx < textChildrenArr.length; idx++) {
        ...
        if (item.nodeName === '#text') {
          ...
          curText = curText.replace(regKeyWord, function (val) {
            var keyword = val.toLowerCase();
            var curKeyVal = eduArr.get(keyword);
            var searchHref = curKeyVal.url + '&spm=1001.2101.3001.7020';
            var reportData = '{"spm":"1001.2101.3001.7020","dest":"' + searchHref + '","extra":"{\\"word\\":\\"' + val + '\\"}"}';
    ...
    var eduKey = getKeywordForEdu(privateEduData) || new Map();
    ...
    if (!isApp && eduKey && eduKey.size > 0) {
      var domList = $("#content_views").find("p,h1,h2,h3,h4,h5,h6");
      $.each(domList, function (i, n) {
        var n = $(n);
        hlDocumentForEdu(eduKey, [].concat(_toConsumableArray(eduKey.keys())), n);
      });
    }
